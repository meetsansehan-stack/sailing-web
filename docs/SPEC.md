...
### 12.7 책장 필터 UI — [설계 확정 2026-06-14, 구현 연기]
**보기 기준 단일축 토글**: `[● 월별 / ○ 주제별]` 세그먼트(반원 pill). 상호배타 — 한 모드만.
- 월별 모드: 월 칩(YYYY-MM) 선택 → 그 달 책.
- 주제별 모드: 주제 칩 선택 → 그 주제 책(**전 기간**, 월 무시). 주제 모드 시 월 disabled.
- intersection 없음(원 스펙 "탭은 해당 월 주제만"은 모순이라 제외 — 주제 모드는 전 기간).

**구현 연기**: 현재 8권·1컬렉션이라 필터가 빈 채로 돌아 시기상조. **≥3 컬렉션 누적 시 구현.** 그때까지 단순 그리드.

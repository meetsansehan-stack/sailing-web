...
- ⓑ **본문 처리 — 남은 것**(10.4): ~~양육자 so-what 콜아웃~~ **✅ 구현(10.7)** / 단계 리스트(신청 단계·방법 → 번호·체크박스) / 지표 강조(시장 지표·근거·데이터 → 수치 비주얼) / 리드 TL;DR(핵심 한 줄·분석 핵심, ⚠️ summary 박스와 중복 주의) = **지금 시드로 구현 가능**, 우선순위 따라.
